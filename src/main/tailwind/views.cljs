   ...
   [:div.rounded-lg.shadow-lg.ring-1.ring-black.ring-opacity-5.overflow-hidden
    [:div.relative.grid.gap-6.bg-white.px-5.py-6
     [:a.-m-4.p-4.block.rounded-md.hover:bg-gray-50.transition.ease-in-out.duration-150 {:href "#"}
      [:p.text-base.font-medium.text-teal-900 "Articles"]
      [:p.mt-1.text-sm.text-gray-500 "Nam non quam efficitur, volutpat erat eu, mattis arcu. "]]
     [:a.-m-4.p-4.block.rounded-md.hover:bg-gray-50.transition.ease-in-out.duration-150 {:href "#"}
      [:p.text-base.font-medium.text-teal-900 "Tutorials"]
      [:p.mt-1.text-sm.text-gray-500 "Pellentesque hendrerit ac velit eget forums of contact support."]]
     [:a.-m-4.p-4.block.rounded-md.hover:bg-gray-50.transition.ease-in-out.duration-150 {:href "#"}
      [:p.text-base.font-medium.text-teal-900 "Help Center"]
      [:p.mt-1.text-sm.text-gray-500 "Nullam tempor malesuada libero, vitae posuere sem lacinia nec."]]]]])

(def framer-motion-params
